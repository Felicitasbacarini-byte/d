...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------------------------------------------------------
   * 3. SCROLL-DRIVEN: el volante se desplaza hacia abajo y crece
   *    hasta el centro mientras se scrollea la sección.
   * --------------------------------------------------------- */
  const wheelStage = document.getElementById('wheel-stage');
  const wheelWrapEl = document.getElementById('wheel-wrap');
  const captionLeft = document.querySelector('.wheel-caption.left');
  const captionRight = document.querySelector('.wheel-caption.right');
  ...
  function updateWheelScroll() {
    ...
    // progreso 0 -> 1 a medida que se atraviesa la sección
    let progress = (-rect.top) / total;
    progress = Math.min(Math.max(progress, 0), 1);

    // el volante arranca desplazado hacia abajo (220px) y chico (0.62)
    // y termina centrado (0px) y grande (1.05)
    const y = 220 - (220 * progress);
    const scale = 0.62 + (0.43 * progress);

    wheelWrapEl.style.setProperty('--wheel-y', `${y}px`);
    wheelWrapEl.style.setProperty('--wheel-scale', scale.toFixed(3));

    // los textos aparecen cuando el volante ya llegó casi al centro
    if (progress > 0.62) {
      captionLeft?.classList.add('is-visible');
      captionRight?.classList.add('is-visible');
    } else {
      captionLeft?.classList.remove('is-visible');
      captionRight?.classList.remove('is-visible');
    }
  }
